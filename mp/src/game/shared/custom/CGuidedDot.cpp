...
//	CGuidedDot *pLaserDot = (CGuidedDot *) CBaseEntity::Create( "guided_dot", origin, QAngle(0,0,0) );
	...
	CGuidedDot *pLaserDot = assert_cast<CGuidedDot*>(CBaseEntity::Create("guided_dot", origin, QAngle(0, 0, 0)));
	...
	pLaserDot->SetSize(-Vector(0, 0, 0), Vector(0, 0, 0));
...
int CGuidedDot::DrawModel( int flags )
{
	...
	{
		scale = Clamp( vecLenght.Length() / 50, 0.5f, 32.0f );
		scale += RandomFloat( -0.5f, 0.5f );
	}
	else
