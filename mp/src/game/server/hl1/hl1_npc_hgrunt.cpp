...
void CNPC_HGrunt::Precache()
{
	...
	// get voice pitch
	if ( RandomInt(0,1))
		m_voicePitch = 109 + RandomInt(0,7);
	else
		m_voicePitch = 100;
...
{
	m_flTalkWaitTime = gpGlobals->curtime + RandomFloat( 1.5f, 2.0f );
	m_iSentence = HGRUNT_SENT_NONE;
}
...
{
	if (FOkToSpeak() && ( g_fGruntQuestion || RandomInt( 0,1 ) ) )
	{
		if (!g_fGruntQuestion)
		{
...
void CNPC_HGrunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_GREN_LAUNCH:
		{
			...
			m_pMyGrenade->SetLocalAngularVelocity( QAngle( RandomFloat( -100, -500 ), 0, 0 ) );
...
void CNPC_HGrunt::Shoot ( void )
{
	...
	Vector	vecShellVelocity = right * RandomFloat(40,90) + up * RandomFloat( 75,200 ) + forward * RandomFloat( -40, 40 );
...
void CNPC_HGruntRepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	pGrunt->SetAbsVelocity( Vector( 0, 0, RandomFloat( -196, -128 ) ) );
