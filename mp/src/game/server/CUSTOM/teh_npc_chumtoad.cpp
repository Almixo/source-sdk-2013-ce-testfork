...
ConVar sk_chumtoad_dmg_bite( "sk_chumtoad_dmg_bite", "0" );
ConVar sk_chumtoad_dmg_pop( "sk_chumtoad_dmg_pop", "0" );
...
#define CHUMTOAD_EXPLOSION_VOLUME	512
#define CHUMTOAD_MODEL "models/chumtoad.mdl"

float CChumToad::m_flNextBounceSoundTime = 0; //bullshit
...
void CChumToad::Precache( void )
{
	...
	PrecacheModel( CHUMTOAD_MODEL );

	PrecacheScriptSound( "Chumtoad.Die" );
	PrecacheScriptSound( "Chumtoad.Gibbed" );
	PrecacheScriptSound( "Chumtoad.Squeak" );
	PrecacheScriptSound( "Chumtoad.Deploy" );
	PrecacheScriptSound( "Chumtoad.Bounce" );
...
void CChumToad::Spawn( void )
{
	...
	SetModel( CHUMTOAD_MODEL );
	UTIL_SetSize( this, Vector( -4, -4, 0 ), Vector( 4, 4, 8 ) );

	SetBloodColor( BLOOD_COLOR_YELLOW );

	SetTouch( &CChumToad::SuperBounceTouch );
	SetThink( &CChumToad::HuntThink );
	...
	SetGravity( UTIL_ScaleForGravity( 400 ) );	// use a lower gravity for chumtoads
	SetFriction( 0.5 );

	SetDamage( sk_chumtoad_dmg_pop.GetFloat() );

	m_flDie = gpGlobals->curtime + CHUMTOAD_DETONATE_DELAY;
...
Class_T	CChumToad::Classify( void )
{
...
void CChumToad::Event_Killed( const CTakeDamageInfo &inputInfo )
{
	//	pev->model = iStringNull;// make invisible
	SetThink( &CChumToad::SUB_Remove );
	SetNextThink( gpGlobals->curtime + 0.1f );
	SetTouch( NULL );
	...
	EmitSound( filter, entindex(), "Chumtoad.Die" );

	CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), CHUMTOAD_EXPLOSION_VOLUME, 3.0 );
...
bool CChumToad::Event_Gibbed( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Chumtoad.Gibbed" );

	return BaseClass::Event_Gibbed( info );
}


void CChumToad::HuntThink( void )
{
	if ( !IsInWorld() )
	{
		SetTouch( NULL );
	...
	{
		CPASAttenuationFilter filter( this );
		EmitSound( filter, entindex(), "Chumtoad.Squeak" );
		CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), 256, 0.25 );
	}

	// higher pitch as squeeker gets closer to detonation time
	float flpitch = 155.0 - 60.0 * ( ( m_flDie - gpGlobals->curtime ) / CHUMTOAD_DETONATE_DELAY );
	if ( flpitch < 80 )
		flpitch = 80;
...
unsigned int CChumToad::PhysicsSolidMaskForEntity( void ) const
{
...
void CChumToad::ResolveFlyCollisionCustom( trace_t &trace, Vector &vecVelocity )
{
...
void CChumToad::SuperBounceTouch( CBaseEntity *pOther )
{
	float	flpitch;
	trace_t tr;
	...
	// higher pitch as squeeker gets closer to detonation time
	flpitch = 155.0 - 60.0 * ( ( m_flDie - gpGlobals->curtime ) / CHUMTOAD_DETONATE_DELAY );
	...
	if ( pOther->m_takedamage && m_flNextAttack < gpGlobals->curtime )
	{
		...
		if ( tr.m_pEnt == pOther )
		{
			...
			if ( tr.m_pEnt->GetModelIndex() != GetModelIndex() )
			{
				...
				if ( m_hOwner != NULL )
				{
					CTakeDamageInfo info( this, m_hOwner, sk_chumtoad_dmg_bite.GetFloat(), DMG_SLASH );
					CalculateMeleeDamageForce( &info, vecForward, tr.endpos );
					pOther->DispatchTraceAttack( info, vecForward, &tr );
				}
				else
				{
					CTakeDamageInfo info( this, this, sk_chumtoad_dmg_bite.GetFloat(), DMG_SLASH );
					CalculateMeleeDamageForce( &info, vecForward, tr.endpos );
					pOther->DispatchTraceAttack( info, vecForward, &tr );
				}
				...
				SetDamage( GetDamage() + sk_chumtoad_dmg_pop.GetFloat() ); // add more explosion damage
				// m_flDie += 2.0; // add more life

				// make bite sound
				CPASAttenuationFilter filter( this );
				CSoundParameters params;
				if ( GetParametersForSound( "Chumtoad.Deploy", params, NULL ) )
				{
	...
	if ( !( GetFlags() & FL_ONGROUND ) )
	{
		...
		CSoundParameters params;
		if ( GetParametersForSound( "Chumtoad.Bounce", params, NULL ) )
		{
			EmitSound_t ep( params );
			ep.m_nPitch = (int)flpitch;
